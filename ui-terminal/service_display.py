...
import requests
from constants import DISPLAY_WAIT_TIME

parser = argparse.ArgumentParser(description="Service Display")
parser.add_argument('--netwatcher', default="localhost:5001", help='Hostname/IP and port of the server')
...
def get_services():
    ...
    try:
        response = requests.get(f"http://{netwatcher}/services")
